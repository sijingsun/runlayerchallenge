...
type ConnectState = "connected" | "disconnected";
...
const CONNECTORS: Connector[] = [
  { id: "slack", name: "Slack · Read + Write · All teams", description: "Slack MCP server for team communication", icon: "/icons/slack.svg", hosting: "hosted", state: "connected", tools: 16, resources: 3 },
  { id: "figma", name: "Figma · Read only · Design", description: "Figma MCP server for design-to-code workflows, component extraction, and design system access.", icon: "/icons/figma.svg", hosting: "hosted", state: "connected", tools: 6, resources: 3 },
  { id: "figma-2", name: "Figma · Read + Write · Product", description: "Figma MCP server for design-to-code workflows, component extraction, and design system access.", icon: "/icons/figma-2.svg", hosting: "hosted", state: "connected", tools: 6, resources: 3 },
  { id: "github", name: "GitHub · Read + Write · Engineering", description: "GitHub MCP server for repository access, pull requests, and code review workflows.", icon: "/icons/github.svg", hosting: "local", state: "connected", tools: 11, prompts: 3 },
  { id: "linear", name: "Linear · Read + Write · Engineering", description: "Linear MCP server for issue tracking, project management, and engineering cycles.", icon: "/icons/linear.svg", hosting: "hosted", state: "connected", tools: 11, prompts: 3 },
  { id: "notion", name: "Notion", description: "Notion MCP server for documents and knowledge management", icon: "/icons/notion.svg", hosting: "hosted", state: "disconnected" },
  { id: "amazon", name: "AWS", description: "AWS MCP server for cloud resource management", icon: "/icons/amazon.svg", hosting: "hosted", state: "disconnected", tools: 15 },
  { id: "asana", name: "Asana", description: "Asana MCP server for project management, task tracking, and team collaboration.", icon: "/icons/asana.svg", hosting: "hosted", state: "disconnected", tools: 16, prompts: 2, resources: 3 },
  { id: "asana-2", name: "Asana #2", description: "Asana MCP server for project management, task tracking, and team collaboration.", icon: "/icons/asana-2.svg", hosting: "hosted", state: "disconnected", tools: 8, prompts: 1, resources: 4 },
  { id: "loom", name: "Loom", description: "Loom MCP server for video recording access, transcripts, and workspace libraries.", icon: "/icons/loom.svg", hosting: "hosted", state: "disconnected", tools: 16, prompts: 2, resources: 3 },
  { id: "salesforce", name: "Salesforce", description: "Salesforce MCP server for CRM data, accounts, opportunities, and customer records.", icon: "/icons/salesforce.svg", hosting: "hosted", state: "disconnected", tools: 16, prompts: 2, resources: 3 },
  { id: "clickup", name: "ClickUp", description: "ClickUp MCP server for task management, spaces, goals, and project workflows.", icon: "/icons/clickup.svg", hosting: "local", state: "disconnected", tools: 11, prompts: 3 },
  { id: "box", name: "Box", description: "Box MCP server for enterprise file storage, folders, and content workflows.", icon: "/icons/box.svg", hosting: "hosted", state: "disconnected", tools: 16, prompts: 2, resources: 3 },
  { id: "vanta", name: "Vanta", description: "Vanta MCP server for compliance status, controls, and security monitoring data.", icon: "/icons/vanta.svg", hosting: "hosted", state: "disconnected", tools: 16, prompts: 2, resources: 3 },
];
...
function ConnectorCard({ connector, showManage }: { connector: Connector; showManage?: boolean }) {
  ...
  return (
    ...
    >
      ...
      <div className="px-4 pb-4 mt-auto flex items-center justify-between gap-2">
        ...
        ) : connector.state !== "connected" && !hasStats ? (
          <button className="px-3 h-[22px] rounded-md text-xs font-semibold"
      ...
      <div className="flex items-center justify-between px-5 shrink-0" style={{ height: "52px", borderBottom: "1px solid rgba(28,25,23,0.07)" }}>
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="flex items-center gap-1.5 px-3 h-[28px] rounded-lg text-xs font-semibold"
            style={{ background: "#1c1917", color: "#fafaf9", border: "none", cursor: "pointer" }}
          >
        ...
        <div className="px-5 py-5">
          ...
          <div className="grid grid-cols-4 gap-4">
            {filteredConnectors.map(c => (
              <ConnectorCard key={c.id} connector={c} showManage={c.state === "connected"} />
            ))}
          </div>
      ...
      <AnimatePresence>
        {panelOpen && (
          <RequestNewPanel
            ...
            onSelectVersion={(connector, version) => {
              setPanelOpen(false);
              setModalConnector({ name: version.name || connector.name, icon: connector.icon });
            }}
            onRequestNew={(_connector) => { setPanelOpen(false); }}
          />
        )}
      </AnimatePresence>
      ...
      <AnimatePresence>
        {modalConnector && (
          <RequestAccessModal
            ...
            connectorIcon={modalConnector.icon}
            onClose={() => setModalConnector(null)}
            onSubmitted={() => setModalConnector(null)}
          />
        )}
      </AnimatePresence>
    </div>
